...
for i=1:fileNum_VD
    ...
    [Wb_VD_out,VD_RGB,VD_Wb_VC]=CMOS2RGB(CMOS_WL,VD_sor0,Cor_ind,Sol_Inc,'mannual',Wb_VD);%mannual代表手动值（如来自每个月昼的定标板的白平衡数值）
    ...
    %以自身进行自动白平衡
    subplot(2,2,4)
    [Wb_VD_auto,VD_RGB,VD_Wb_self]=CMOS2RGB(CMOS_WL,VD_sor0,Cor_ind,Sol_Inc,'self');%[Wb,C_RGB,C_RGB_Wb]=CMOS2RGB(CMOS_WL,CMOS_dat,Cor_ind,WB_mod,Wb_in),'self'代表以自身自动白平衡
    ...
    if strcmpi(Day_normal,'Y')
        VD_none=VD_RGB./0.007; %除以0.1表示，以最大亮度0.1进行拉伸
        VD_Wb_VC_export=VD_Wb_VC./0.007;
        Band_i=Band_i./0.04;
    else
